use crate::{
    model::{DownloadCapture, IngestResult},
    phase4, storage,
};
...
fn backfill_locations(connection: &Connection) -> rusqlite::Result<()> {
    ...
    for (hash, path, state) in rows {
        connection.execute(
            ...
            "#,
            params![hash, path, location_state(&state)],
        )?;
    }
    Ok(())
}
...
pub fn record_capture(
    ...
    connection
        .execute(
            ...
            params![
                result.id,
                clean_optional(capture.browser_name.as_deref(), 80),
                clean_optional(capture.page_title.as_deref(), 500),
                clean_optional(capture.page_url.as_deref(), 4096),
                clean_optional(capture.link_text.as_deref(), 500),
                clean_optional(capture.context_text.as_deref(), 2000),
                clean_optional(capture.context_source.as_deref(), 80),
            ],
        )
...
pub fn list_passport_summaries(path: &Path) -> Result<Vec<PassportSummary>, String> {
    ...
    let mut statement = connection
        .prepare(
            r#"
            SELECT d.id,
                   COALESCE(pc.purpose, 'UNSPECIFIED'),
                   pc.expires_at, pc.page_title, pc.page_url,
...
fn list_locations(connection: &Connection, hash: &str) -> Result<Vec<FileLocation>, String> {
    let mut statement = connection
        .prepare(
            r#"
            SELECT path, state, first_seen_at, last_seen_at
            FROM file_locations
            WHERE sha256 = ?1
            ORDER BY state DESC, last_seen_at DESC, path ASC
            "#,
        )
        .map_err(|error| error.to_string())?;
    statement
...
fn list_trust(connection: &Connection, download_id: i64) -> Result<Vec<TrustObservation>, String> {
    let mut statement = connection
        .prepare(
            r#"
            SELECT kind, state, summary, details, checked_at
...
pub fn update_passport_metadata(
    ...
    let note = clean_optional(note.as_deref(), 4000);
    let expires_at = clean_optional(expires_at.as_deref(), 80);
    let sigstore_identity = clean_optional(sigstore_identity.as_deref(), 512);
    let sigstore_issuer = clean_optional(sigstore_issuer.as_deref(), 1024);
    let connection = Connection::open(path).map_err(|error| error.to_string())?;
...
pub fn reconnect_file(
    ...
    let candidate_hash = storage::sha256_file(&candidate).map_err(|error| error.to_string())?;
    ...
    if primary_path_updated {
        ...
        transaction
            .execute(
                ...
                params![candidate.display().to_string(), file_name, download_id],
            )
            .map_err(|error| error.to_string())?;
        transaction
            .execute(
...
pub fn export_passport(path: &Path, download_id: i64) -> Result<PassportExportResult, String> {
    ...
    let source_path = usable_file_path(&passport)
        .ok_or_else(|| "No local or recoverable archive copy is available for passport export".to_string())?;
    let exported_at = current_timestamp(path)?;
...
pub fn import_passport(path: &Path, sidecar_path: String) -> Result<FilePassport, String> {
    ...
    record_capture(path, &capture, &result)?;
    let connection = Connection::open(path).map_err(|error| error.to_string())?;
    connection
        .execute(
            r#"
            UPDATE passport_context
            SET purpose = ?1, note = ?2, expires_at = ?3, imported_from = ?4,
                updated_at = CURRENT_TIMESTAMP
            WHERE download_id = ?5
            "#,
            params![
...
pub fn refresh_trust(path: &Path, download_id: i64) -> Result<Vec<TrustObservation>, String> {
    ...
    let mut observations = vec![integrity_observation(&file_path, passport.sha256.as_deref())];
    observations.push(mark_of_web_observation(&file_path));
    observations.push(authenticode_observation(&file_path));
    observations.push(c2pa_observation(&file_path));
    observations.push(sigstore_observation(
        &file_path,
...
fn mark_of_web_observation(path: &Path) -> TrustObservation {
    ...
    match fs::read_to_string(&ads) {
        Ok(content) => {
            let zone = ini_value(&content, "ZoneId").unwrap_or_else(|| "unknown".into());
            let host = ini_value(&content, "HostUrl");
            let referrer = ini_value(&content, "ReferrerUrl");
            observation(
                "WINDOWS_ORIGIN",
                "PRESENT",
                ...
                Some(format!(
                    "HostUrl: {}; ReferrerUrl: {}",
                    host.as_deref().unwrap_or("unavailable"),
                    referrer.as_deref().unwrap_or("unavailable")
                )),
            )
        }
...
fn ini_value(content: &str, key: &str) -> Option<String> {
    content.lines().find_map(|line| {
        ...
        (candidate.trim().eq_ignore_ascii_case(key)).then(|| value.trim().to_string())
    })
}
...
fn authenticode_observation(path: &Path) -> TrustObservation {
        ...
        .output()
    {
        Ok(output) if output.status.success() => {
            ...
            let signer = value.get("Signer").and_then(Value::as_str);
            match status {
                "Valid" => observation(
                    "AUTHENTICODE",
                    "VERIFIED",
                    "Windows Authenticode signature is valid.".into(),
                    signer.map(ToOwned::to_owned),
                ),
    ...
    let output = match Command::new("c2patool").arg(path).arg("--info").output() {
        ...
        Err(error) if error.kind() == ErrorKind::NotFound => {
            return observation(
                "C2PA",
                "VERIFIER_UNAVAILABLE",
                "C2PA verification is available when the official c2patool executable is installed.".into(),
                None,
            )
        }
        Err(error) => {
            return observation(
                "C2PA",
                "CHECK_FAILED",
                "OriginKeep could not invoke the C2PA verifier.".into(),
                Some(error.to_string()),
            )
        }
    };
...
fn non_empty(value: String) -> Option<String> {
    (!value.is_empty()).then_some(value.chars().take(4000).collect())
}
...
fn sigstore_observation(
    ...
    if !bundle.is_file() {
        return observation(
            "SIGSTORE",
            "NO_BUNDLE",
            "No adjacent Sigstore bundle was found.".into(),
    ...
    let (Some(identity), Some(issuer)) = (expected_identity, expected_issuer) else {
        return observation(
            "SIGSTORE",
            "POLICY_REQUIRED",
            "A Sigstore bundle is present; set the expected signing identity and OIDC issuer before verification.".into(),
...
pub fn origin_graph(path: &Path) -> Result<OriginGraph, String> {
    ...
    let rows = {
        let mut statement = connection
            .prepare(
                r#"
                SELECT id, file_name, sha256, source_identity, version_number, duplicate_of_id
                FROM downloads
                ORDER BY COALESCE(source_identity, ''), COALESCE(version_number, 0), id
                "#,
            )
            .map_err(|error| error.to_string())?;
        statement
    ...
    let mut prior_by_source: std::collections::BTreeMap<String, (i64, String)> =
...
mod tests {
    ...
    #[test]
    fn location_refresh_marks_missing_paths() {
        let db = env::temp_dir().join("originkeep-passport-location-test.db");
        fs::remove_file(&db).ok();
        initialize_database(&db).unwrap();
        let connection = Connection::open(&db).unwrap();
        upsert_location(&connection, "deadbeef", "/definitely/not/present", "PRESENT").unwrap();
        let summary = refresh_locations(&db).unwrap();
        fs::remove_file(&db).ok();
